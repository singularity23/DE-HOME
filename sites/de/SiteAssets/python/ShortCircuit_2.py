    ...
    def store_info(self, SCReport) -> None:
        ...
        if self.DevType == 11:  # OverheadLine
            ...
            _write_info(
                "Impedances Note:",
                textwrap.fill(
                    str(self.ImpNote), 80, initial_indent="", subsequent_indent=" " * 26
                ),  # type: ignore
            )
...
class TransformerEquipment(BaseEquipment):
    ...
    def _validate_impedance_calculations(self) -> None:
        ...
        if not math.isclose(
            self.R1c**2 + self.R1**2,
            2 * self.R1c * self.R1,
            rel_tol=1e-4,  # type: ignore
        ) and not math.isclose(
            self.R0c**2 + self.R0**2,
            2 * self.R0c * self.R0,
            rel_tol=1e-4,  # type: ignore
        ):
...
class EmissionStudy(ShortCircuitStudy):
    ...
    def _calculate_loads(self) -> None:
        ...
        self.estimated_slv = round(
            round(self.slv_percentage, 2) * self.parameters.feeder_limit,
            3,  # type: ignore
        )
