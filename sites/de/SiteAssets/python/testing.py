...
# Version 1.2 - Revised 2020-Nov-20 by Kan Tang and Paul Therrien to correct Source Equivalent issue when User Defined Equivalent impedances are used. Also removed 'INT_WIRE' entries from
...
class EmissionStudy:
		...
		def __init__(self, PCC, network_id, distance, R1, X1, R0, X0):
				self.PCC = PCC
...
if cympy.study.QueryInfoNode('$NodeId$', 'FAULT_POINT') == '':
	...
	if cympy.study.GetDevice(network_id, cympy.enums.DeviceType.Source) is None:
		...
		Source_Name = cympy.study.QueryInfoNode('$UpstreamSource$', 'FAULT_POINT')

	# remove the underscore from the network id to align with BCH file name requirements
	Source_Name_filename = Source_Name.replace("_", "-")
	Report_FileName = "SC Report " + Source_Name_filename + \
	    " " + current_datetime + ".txt"
	Rep_Loc = cympy.GetInputParameter('Report_Location') + Report_FileName

	if os.path.exists(Rep_Loc):  # in case file already exists, delete all file contents
	...
	# Append list header
	NetworkParam.append(["Equipment Type", "Length(m)", "Dist. to Sub(" + UnitText[units]
	                    [UnitSize] + ")", "R1", "X1", "R0", "X0", "Thev_R1", "Thev_X1", "Thev_R0", "Thev_X0"])
	...
	if int(sc.GetValue('ParametersConfigurations')) > 1:
		while sc.GetValue('ActiveConfigurationID') != sc.GetValue('ParametersConfigurations['+str(i)+'].ConfigID'):
			i += 1
	...
	if sc.GetValue('ParametersConfigurations['+str(i)+'].Domain') == 'FF':
		...
		sc.SetValue('SC', 'ParametersConfigurations['+str(i)+'].Domain')
	sc.Run()
	...
	Fault_to_Sub_Dist = float(
	    cympy.study.QueryInfoNode('$Distance$', 'FAULT_POINT', 3))
	XCoord = cympy.study.QueryInfoNode('$CoordX$', 'FAULT_POINT')
	YCoord = cympy.study.QueryInfoNode('$CoordY$', 'FAULT_POINT')
	NetworkParam.append(['Fault Point (XCoord:' + str(XCoord) + ' YCoord:' + str(
	    YCoord) + ')', 0, Fault_to_Sub_Dist, 0, 0, 0, 0, R1ohm, X1ohm, R0ohm, X0ohm])
	i = 1  # NetworkParam list iterator, i=1 is the FAULT_POINT entry
	Dist_to_Sub = Fault_to_Sub_Dist
 
 
 	R1pcc = float(cympy.study.QueryInfoNode('$R1ohm$', PCC))
	X1pcc = float(cympy.study.QueryInfoNode('$X1ohm$', PCC))
	R0pcc = float(cympy.study.QueryInfoNode('$R0ohm$', PCC))
	X0pcc = float(cympy.study.QueryInfoNode('$X0ohm$', PCC))
	PCC_distance = float(
	    cympy.study.QueryInfoNode('$Distance$', PCC, 3))

	PCC = cympy.GetInputParameter("PCC")
	ES = EmissionStudy(PCC, Source_Name, PCC_distance, R1pcc, X1pcc, R0pcc, X0pcc)
				ES.ReadInputs()
	...
	if len(cympy.study.ListDevices(cympy.enums.DeviceType.SynchronousGenerator,network_id))>0 or len(cympy.study.ListDevices(cympy.enums.DeviceType.InductionGenerator,network_id))>0 or len(cympy.study.ListDevices(cympy.enums.DeviceType.ElectronicConverterGenerator,network_id))>0:
		print("**WARNING**")
	...
	iterator = cympy.study.NetworkIterator('FAULT_POINT', cympy.enums.IterationOption.Upstream, cympy.enums.IterationRestriction.StopOnOpen)
	while ( iterator.Next() ):
		Sections = iterator.GetSection()
		DeviceList = cympy.study.ListDevices(cympy.enums.DeviceType.AllDevices,".*","^"+Sections.ID+"$") #list of all device attached to the section in the current iteration
		...
		#this is the ensure that the correct portion of either OH or UG line is summed on the correct side of the Xfmr/reactor			
		for Device in DeviceList:
			if Device.DeviceType == 1 or Device.DeviceType == 33 or Device.DeviceType == 42 or Device.DeviceType == 9: #Transformer, by-phase transformer, autotransformer, or series reactor
				if Device.Location == 0 or Device.Location == 1: #if device is at From node (source side of section) or in the middle of the section
					DeviceList.append(DeviceList.pop(DeviceList.index(Device)))# move to end of list
				elif Device.Location == 2: #if device is at To node (load side of section)
					DeviceList.insert(0,DeviceList.pop(DeviceList.index(Device)))# move to start of list

		# Thevenin Impedance at Load side of current Section
		R1ohm = float(cympy.study.QueryInfoNode('$R1ohm$',Sections.ToNode.ID))
		X1ohm = float(cympy.study.QueryInfoNode('$X1ohm$',Sections.ToNode.ID))
		R0ohm = float(cympy.study.QueryInfoNode('$R0ohm$',Sections.ToNode.ID))
		X0ohm = float(cympy.study.QueryInfoNode('$X0ohm$',Sections.ToNode.ID))
		...
		for Device in DeviceList:
			if Device.DeviceType == 1 or Device.DeviceType == 42: #Two-Winding Step up/down Transformer and Autotransformers
				XfmrType = cympy.study.QueryInfoDevice('$EqCode$', Device.DeviceNumber, Device.DeviceType)
				XfmrID = cympy.study.QueryInfoDevice('$EqId$',Device.DeviceNumber, Device.DeviceType)
				XfmrKVA = float(cympy.study.QueryInfoDevice('$XfoKVANomTot$',Device.DeviceNumber,Device.DeviceType))
				Z1per = float(cympy.study.QueryInfoDevice('$XfoZ1$',Device.DeviceNumber,Device.DeviceType))
				Z0per = float(cympy.study.QueryInfoDevice('$XfoZ0$',Device.DeviceNumber,Device.DeviceType))
				X1R1 = float(cympy.study.QueryInfoDevice('$XfoX1R1Ratio$',Device.DeviceNumber,Device.DeviceType))
				X0R0 = float(cympy.study.QueryInfoDevice('$XfoX0R0Ratio$',Device.DeviceNumber,Device.DeviceType))
				PrimVolts = float(cympy.study.QueryInfoDevice('$XfoKVLL1$',Device.DeviceNumber,Device.DeviceType))
				SecVolts = float(cympy.study.QueryInfoDevice('$XfoKVLL2$',Device.DeviceNumber,Device.DeviceType))
				...
				X0 = X0R0*R0
				EquipmentList.append([Device.EquipmentID, Device.DeviceType, Device.DeviceNumber])
				NetworkParam.append([str(XfmrType) + ': ' + str(XfmrID),0.0,Dist_to_Sub,R1,X1,R0,X0,R1ohm,X1ohm,R0ohm,X0ohm])
				i = i + 1
			
			elif Device.DeviceType == 33:	#By-Phase Transformer
				XfmrType = cympy.study.QueryInfoDevice('$EqCode$', Device.DeviceNumber, Device.DeviceType)
				XfmrIDA = cympy.study.QueryInfoDevice('$XfoByPhaseEqIdA$',Device.DeviceNumber, Device.DeviceType)
				XfmrIDB = cympy.study.QueryInfoDevice('$XfoByPhaseEqIdB$',Device.DeviceNumber, Device.DeviceType)
				XfmrIDC = cympy.study.QueryInfoDevice('$XfoByPhaseEqIdC$',Device.DeviceNumber, Device.DeviceType)
				XfmrKVA = float(cympy.study.QueryInfoDevice('$XfoByPhaseKVANomTot$',Device.DeviceNumber,Device.DeviceType))
				Z1per = float(cympy.study.QueryInfoDevice('$XfoByPhaseZ1$',Device.DeviceNumber,Device.DeviceType))
				Z0per = float(cympy.study.QueryInfoDevice('$XfoByPhaseZ0$',Device.DeviceNumber,Device.DeviceType))
				X1R1 = float(cympy.study.QueryInfoDevice('$XfoByPhaseX1R1Ratio$',Device.DeviceNumber,Device.DeviceType))
				X0R0 = float(cympy.study.QueryInfoDevice('$XfoByPhaseX0R0Ratio$',Device.DeviceNumber,Device.DeviceType))
				PrimVolts = float(cympy.study.QueryInfoDevice('$XfoByPhaseKvPrimA$',Device.DeviceNumber,Device.DeviceType))
				SecVolts = float(cympy.study.QueryInfoDevice('$XfoByPhaseKvSecA$',Device.DeviceNumber,Device.DeviceType))
				XfZ1ohms_Mag = (Z1per*SecVolts*SecVolts*10)/XfmrKVA
				XfZ0ohms_Mag = (Z0per*SecVolts*SecVolts*10)/XfmrKVA
				R1 = math.sqrt(XfZ1ohms_Mag*XfZ1ohms_Mag/(1+(X1R1*X1R1)))
				X1 = X1R1*R1
				R0 = math.sqrt(XfZ0ohms_Mag*XfZ0ohms_Mag/(1+(X0R0*X0R0)))
				X0 = X0R0*R0
				EquipmentList.append(["By-Phase Xfmr", Device.DeviceType, Device.DeviceNumber])
				NetworkParam.append([str(XfmrType) + ': A:' + str(XfmrIDA) +' B:'+str(XfmrIDB)+' C:'+str(XfmrIDC),0.0,Dist_to_Sub,R1,X1,R0,X0,R1ohm,X1ohm,R0ohm,X0ohm])
				i = i + 1
			
			elif Device.DeviceType == 9:	#Series Reactor
				R1 = R1ohm - float(cympy.study.QueryInfoNode('$R1ohm$',Sections.FromNode.ID))
				X1 = X1ohm - float(cympy.study.QueryInfoNode('$X1ohm$',Sections.FromNode.ID))
				R0 = R0ohm - float(cympy.study.QueryInfoNode('$R0ohm$',Sections.FromNode.ID)) #calculate the impedance difference between ToNode and FromNode
				X0 = X0ohm - float(cympy.study.QueryInfoNode('$X0ohm$',Sections.FromNode.ID))
				...
				if any(Device.DeviceNumber in Eq for Eq in EquipmentList) and NetworkParam[i][0] == ("Series Reactor: " + str(Device.EquipmentID)):
					NetworkParam[i][3] = R1									#assumes the duplicate reactor is within the previously iterated section
					...
					EquipmentList.append([Device.EquipmentID, Device.DeviceType, Device.DeviceNumber])
					NetworkParam.append(["Series Reactor: " + str(Device.EquipmentID),0,0,R1,X1,R0,X0,R1ohm,X1ohm,R0ohm,X0ohm])
					i = i + 1

			elif Device.DeviceType == 10: # UG Cable
				LineID = "UG Cable: " + str(Device.EquipmentID)
				Length = Sections.Length
				Dist_to_Sub = float(cympy.study.QueryInfoNode('$Distance$',Sections.FromNode.ID,3))

				if cympy.study.QueryInfoDevice('$PhaseCount$',Device.DeviceNumber,Device.DeviceType) == '1': #calculate the impedance at the From node
					# 1ph cable impedances
					R1FN = float(cympy.study.QueryInfoNode('$R1ohm$',Sections.FromNode.ID))
					X1FN = float(cympy.study.QueryInfoNode('$X1ohm$',Sections.FromNode.ID))
					R0FN = float(cympy.study.QueryInfoNode('$R0ohm$',Sections.FromNode.ID))
					X0FN = float(cympy.study.QueryInfoNode('$X0ohm$',Sections.FromNode.ID))
					R1 = ((R1FN + R1FN + R0FN)/3)
					X1 = ((X1FN + X1FN + X0FN)/3)
					R0 = ((R1FN + R1FN + R0FN)/3)
					X0 = ((X1FN + X1FN + X0FN)/3)
				else: # 2ph and 3ph cable impedances
					R1 = float(cympy.study.QueryInfoNode('$R1ohm$',Sections.FromNode.ID))
					X1 = float(cympy.study.QueryInfoNode('$X1ohm$',Sections.FromNode.ID))
					R0 = float(cympy.study.QueryInfoNode('$R0ohm$',Sections.FromNode.ID))
					X0 = float(cympy.study.QueryInfoNode('$X0ohm$',Sections.FromNode.ID))		
				...
				if NetworkParam[i][0] == LineID: #same as last section
					...
					NetworkParam.append([LineID,Length,Dist_to_Sub,R1ohm - R1,X1ohm - X1,R0ohm - R0,X0ohm - X0,R1ohm,X1ohm,R0ohm,X0ohm])
					i = i + 1
				else:
				...
				if NetworkParam[i][0] == LineID: #same as the last section so continue to sum
					NetworkParam[i][1] = NetworkParam[i][1] + Length
					NetworkParam[i][2] = Dist_to_Sub
					NetworkParam[i][3] = NetworkParam[i][7] - R1
					NetworkParam[i][4] = NetworkParam[i][8] - X1
		...
		if cympy.study.QueryInfoNode('$IsSourceNode$',Sections.FromNode.ID) == 'Yes' or cympy.study.QueryInfoNode('$IsSourceNode$',Sections.ToNode.ID) == 'Yes':
	...
	Level = cympy.study.QueryInfoNode('$SourceFaultLevel$',Sections.FromNode.ID) #Source Equivalent Fault Level Used (ie. high or low)

	EquipmentList.append([Source_Name,'Source'])
	NetworkParam.append(["Source Equivalent: "+str(Source_Name),0,0,R1ohm,X1ohm,R0ohm,X0ohm,R1ohm,X1ohm,R0ohm,X0ohm])

	#print out the NetworkParam entries as a table to the Cyme console and the Report text file
	textfile = open(Rep_Loc,"w")
	textfile.write('Circuit: ' + str(network_id) + '          |---Device Impedance (ohms)--||From Load side of Device (ohms)|')
	print('Circuit: ' + str(network_id) + '	          |---Device Impedance (ohms)--||---From Load side of Device---|')

	separator = "—"*89
	for row in NetworkParam:
		if (row[0] != NetworkParam[0][0]) and (not "INT_WIRE" in row[0]): #not the header row and not an 'INT_WIRE' cable section
			print(row[0])
			print('{:<10.2f}{:<15.1f}{:>8.4f}{:>8.4f}{:>8.4f}{:>8.4f}{:>8.4f}{:>8.4f}{:>8.4f}{:>8.4f}'.format(row[1],row[2],row[3],row[4],row[5],row[6],row[7],row[8],row[9],row[10]))
			textfile.write('\n' + row[0] + '\n')
			textfile.write('{:<10.2f}{:<15.1f}{:>8.4f}{:>8.4f}{:>8.4f}{:>8.4f}{:>8.4f}{:>8.4f}{:>8.4f}{:>8.4f}'.format(row[1],row[2],row[3],row[4],row[5],row[6],row[7],row[8],row[9],row[10]))
		elif row[0] == NetworkParam[0][0]: #header row
			print('{:<10}{:<15}{:>8}{:>8}{:>8}{:>8}{:>8}{:>8}{:>8}{:>8}'.format(row[1],row[2],row[3],row[4],row[5],row[6],row[7],row[8],row[9],row[10]))
			print(separator)
			textfile.write('\n{:<10}{:<15}{:>8}{:>8}{:>8}{:>8}{:>8}{:>8}{:>8}{:>8}'.format(row[1],row[2],row[3],row[4],row[5],row[6],row[7],row[8],row[9],row[10]))
			textfile.write('\n' + separator)
	print(separator)
	...
	textfile.write('\nFault Impedance:')
	textfile.write('\n        R     X')
	textfile.write('\n{:<8}{:<6}{:<6} ohms'.format('Zf-LLL',LLL_Fault_Resistance,LLL_Fault_Reactance))
	textfile.write('\n{:<8}{:<6}{:<6} ohms'.format('Zg-LG',LG_Fault_Resistance,LG_Fault_Reactance))
	...
	for Eq in EquipmentList:
		if Eq[1] == 1 or Eq[1] == 42: #Two-Winding Transformer and Autotransformer
			...
			if Type == 'Single-phase':
				textfile.write('\n' + str(Type) + ' x ' + str(cympy.study.QueryInfoDevice('$XfoKVANom$',Eq[2],Eq[1])) + ' kVA')
			else:
				textfile.write('\nType: ' + str(Type))
			textfile.write('\nTotal Nominal Bank Rating: ' + str(float(cympy.study.QueryInfoDevice('$XfoKVANomTot$',Eq[2],Eq[1]))) + ' kVA')
			textfile.write('\n' + str(PrimVolts) + 'kVLL x ' + str(SecVolts) + 'kVLL')
			textfile.write('\nWinding Configuration: ' + str(cympy.study.QueryInfoDevice('$XfoConn$',Eq[2],Eq[1])))
			textfile.write('\nZ1: ' + str(Z1per) + '%, Z0: ' + str(Z0per) + '%')
			textfile.write('\nX1/R1: ' + str(X1R1) + ', X0/R0: ' + str(X0R0) + '\n')
	
		elif Eq[1] == 33:	#By-Phase Transformers
			textfile.write('\nBY-PHASE TRANSFORMER')
			PrimVolts = float(cympy.study.QueryInfoDevice('$XfoByPhaseVBaseFrom$',Eq[2],Eq[1]))
			SecVolts = float(cympy.study.QueryInfoDevice('$XfoByPhaseVBaseTo$',Eq[2],Eq[1]))
			textfile.write('\n' + str(PrimVolts) + 'kVLL -> ' + str(SecVolts) + 'kVLL')
			textfile.write('\nWinding Configuration: ' + str(cympy.study.QueryInfoDevice('$XfoByPhaseConnection$',Eq[2],Eq[1])))
			...
			if TypeA == 'Single-phase':
				...
				Z1perA = float(cympy.study.QueryInfoDevice('$XfoByPhaseZ1A$',Eq[2],Eq[1]))
				Z0perA = float(cympy.study.QueryInfoDevice('$XfoByPhaseZ0A$',Eq[2],Eq[1]))
				X1R1_A = float(cympy.study.QueryInfoDevice('$XfoByPhaseX1R1RatioA$',Eq[2],Eq[1]))
				X0R0_A = float(cympy.study.QueryInfoDevice('$XfoByPhaseX0R0RatioA$',Eq[2],Eq[1]))
				textfile.write('\nA phase: [' + str(XfmrIDA) + ']')
				textfile.write('\nNominal Power Rating: ' + str(float(cympy.study.QueryInfoDevice('$XfoByPhaseKVANomA$',Eq[2],Eq[1]))) + ' kVA')
			...
			if TypeB == 'Single-phase':
				...
				Z1perB = float(cympy.study.QueryInfoDevice('$XfoByPhaseZ1B$',Eq[2],Eq[1]))
				Z0perB = float(cympy.study.QueryInfoDevice('$XfoByPhaseZ0B$',Eq[2],Eq[1]))
				X1R1_B = float(cympy.study.QueryInfoDevice('$XfoByPhaseX1R1RatioB$',Eq[2],Eq[1]))
				X0R0_B = float(cympy.study.QueryInfoDevice('$XfoByPhaseX0R0RatioB$',Eq[2],Eq[1]))
				textfile.write('\nB phase: [' + str(XfmrIDB) + ']')
				textfile.write('\nNominal Power Rating: ' + str(float(cympy.study.QueryInfoDevice('$XfoByPhaseKVANomB$',Eq[2],Eq[1]))) + ' kVA')
			...
			if TypeC == 'Single-phase':
				...
				Z1perC = float(cympy.study.QueryInfoDevice('$XfoByPhaseZ1C$',Eq[2],Eq[1]))
				Z0perC = float(cympy.study.QueryInfoDevice('$XfoByPhaseZ0C$',Eq[2],Eq[1]))
				X1R1_C = float(cympy.study.QueryInfoDevice('$XfoByPhaseX1R1RatioC$',Eq[2],Eq[1]))
				X0R0_C = float(cympy.study.QueryInfoDevice('$XfoByPhaseX0R0RatioC$',Eq[2],Eq[1]))
				textfile.write('\nC phase: [' + str(XfmrIDC) + ']')
				textfile.write('\nNominal Power Rating: ' + str(float(cympy.study.QueryInfoDevice('$XfoByPhaseKVANomC$',Eq[2],Eq[1]))) + ' kVA')
			...
			textfile.write('\n[' + str(Eq[0]) + ']')
			textfile.write('\nSeries Reactor')
			textfile.write('\nRated Current: ' + str(cympy.eq.GetValue('RatedCurrent',Eq[0],13)) + ' Amps')
			textfile.write('\nInductive Reactance: ' + str(cympy.eq.GetValue('ReactanceOhms',Eq[0],13)) + ' ohms/phase\n')
	
		elif Eq[1] == 10:	#UG Cable
			textfile.write('\n[' + str(Eq[0]) + ']')
			textfile.write('\nUnderground Cable')
			textfile.write('\nZ1: ' + str(cympy.eq.GetValue('PositiveSequenceResistance',Eq[0],cympy.enums.EquipmentType.Cable)) + ' + j' + str(cympy.eq.GetValue('PositiveSequenceReactance',Eq[0],cympy.enums.EquipmentType.Cable)) + ' ohms/km')
			textfile.write('\nZ0: ' + str(cympy.eq.GetValue('ZeroSequenceResistance',Eq[0],cympy.enums.EquipmentType.Cable)) + ' + j' + str(cympy.eq.GetValue('ZeroSequenceReactance',Eq[0],cympy.enums.EquipmentType.Cable)) + ' ohms/km')
			textfile.write('\nComments: ')
			textfile.write(str(cympy.eq.GetValue('ImpedancesNote',Eq[0],cympy.enums.EquipmentType.Cable)) + '\n')

		elif Eq[1] == 11:	#OH Line
			textfile.write('\n[' + str(Eq[0]) + ']')
			textfile.write('\nOverhead Line')
			textfile.write('\nZ1: ' + str(cympy.eq.GetValue('PositiveSequenceResistance',Eq[0],cympy.enums.EquipmentType.OverheadLine)) + ' + j' + str(cympy.eq.GetValue('PositiveSequenceReactance',Eq[0],cympy.enums.EquipmentType.OverheadLine)) + ' ohms/km')
			textfile.write('\nZ0: ' + str(cympy.eq.GetValue('ZeroSequenceResistance',Eq[0],cympy.enums.EquipmentType.OverheadLine)) + ' + j' + str(cympy.eq.GetValue('ZeroSequenceReactance',Eq[0],cympy.enums.EquipmentType.OverheadLine)) + ' ohms/km')
			textfile.write('\nPhase Conductors: ' + str(cympy.eq.GetValue('PhaseConductorID',Eq[0],cympy.enums.EquipmentType.OverheadLine)))
			textfile.write('\nNeutral Conductor: ' + str(cympy.eq.GetValue('NeutralConductorID',Eq[0],cympy.enums.EquipmentType.OverheadLine)))
			textfile.write('\nConductor Spacing: ' + str(cympy.eq.GetValue('ConductorSpacingID',Eq[0],cympy.enums.EquipmentType.OverheadLine)))
			textfile.write('\nComments: ')
			textfile.write(str(cympy.eq.GetValue('Comments',Eq[0],cympy.enums.EquipmentType.OverheadLine)) + '\n')
	
		elif Eq[1] == 'Source':	#Source Equivalent
			textfile.write('\n[' + str(Eq[0]) + ']')
			textfile.write('\nSource Equivalent')
			textfile.write('\nID: ' + str(Source_Name))
			textfile.write('\nLow Fault Level Z1: ' + str(cympy.study.QueryInfoNode('SourceR1ohmsMax',Sections.FromNode.ID)) + ' + j' + str(cympy.study.QueryInfoNode('SourceX1ohmsMax',Sections.FromNode.ID)) + ' ohms')
			textfile.write('\nLow Fault Level Z0: ' + str(cympy.study.QueryInfoNode('SourceR0ohmsMax',Sections.FromNode.ID)) + ' + j' + str(cympy.study.QueryInfoNode('SourceX0ohmsMax',Sections.FromNode.ID)) + ' ohms')
			textfile.write('\nHigh Fault Level Z1: ' + str(cympy.study.QueryInfoNode('SourceR1ohmsMin',Sections.FromNode.ID)) + ' + j' + str(cympy.study.QueryInfoNode('SourceX1ohmsMin',Sections.FromNode.ID)) + ' ohms')
			textfile.write('\nHigh Fault Level Z0: ' + str(cympy.study.QueryInfoNode('SourceR0ohmsMin',Sections.FromNode.ID)) + ' + j' + str(cympy.study.QueryInfoNode('SourceX0ohmsMin',Sections.FromNode.ID)) + ' ohms')
			textfile.write('\nComments: ')
			try: 
				...
				textfile.write(cympy.eq.GetValue('Comments',Source_Name,cympy.enums.EquipmentType.Substation))
