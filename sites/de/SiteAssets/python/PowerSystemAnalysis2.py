...
class PowerSystemAnalysis:
    ...
    #
    #    Attributes:
    #        Various attributes related to power system analysis like source name, network
    #        ID, nominal voltage, operating voltage, etc.
    #
    #    Methods:
    #        The class includes methods for initializing parameters, reading input, setting up
    #        study environments, configuring simulations, and initializing study variables.
    #    """
    ...
    def setup_study_env(self, fault_point):
        ...
        if source_dev := GetDevice(self.network_id, self.dev_type_source):
            self.set_value_source(set_list, self.source_name)
    ...
    def initialize_study_variables(self):
        ...
        self.rep_loc = self.rep_path + self.report_filename
    ...
    def run(self):
        ...
        for point in ps.points:
            ...
            with open(ps.rep_loc, "w") as textfile:
                ...
                textfile.write("{}\n".format(ps.table_separator))
                textfile.write(ps.table_header)
                print(ps.table_header)
                ...
                (
                    ...
                    LLG_imp,
                    LLG_imp_3Io,
                ) = ps.query_node(ps.fault_current_list, ps.fault_point)
                (
                    PreFaultVolts,
                    R1ohm,
                    X1ohm,
                    ...
                    Latitude,
                    Longitude,
                ) = ps.query_node(ps.variables_list, ps.fault_point)
                ...
                while iterator.Next():
                    ...
                    (
                        ...
                        Ph_Num_UpStream,
                        Parallel_Run,
                    ) = ps.query_node(ps.cable_variables, Node.ID)
                    ...
                    (
                        ...
                        Ph_Num_DownStream,
                        Parallel_Run,
                    ) = ps.query_node(ps.cable_variables, FromNode.ID)
                    ...
                    for Device in DeviceList:
                        ...
                        if (
                            ...
                            ) = ps.query_node(per_unit, Node.ID)
                            ...
                            ) = ps.query_node(per_unit, FromNode.ID)
                        ...
                        # Operating Devices
                            ...
                            Device.DeviceType
                            == 9  # cympy.enums.DeviceType.SeriesReactor
                        ...
                        # UG Cable
                        elif Device.DeviceType == 10 or Device.DeviceType == 11:
                            ...
                            if (
                                not any(
                                    Device.EquipmentID in Eq for Eq in ps.equipment_list
                                )
                    ...
                    if QueryInfoNode("IsSourceNode", Node.ID) == "Yes":
                        ...
                        EqID = "SourceEquivalent: " + str(ps.source_name)
                        ps.equipment_type.append(EqID)
                ...
                for index, param in enumerate(zip(ps.equipment_type, ps.network_param)):
                    if not "INT_WIRE" in param[0]:
                        if index == 0:
                            print(ps.table_header_format.format(
                                param[0], *param[1]))
                            print("\n{}".format(ps.table_separator))
                            textfile.write(
                            ...
                            # print(param[10])
                            print(ps.table_row_format.format(
                                param[0], *param[1]))
                            textfile.write(
                                ("\n" +
                ...
                print("\n{:<25} {}".format(
                    "Detailed Report Here:", ps.rep_loc))
                print("\n{}".format(ps.table_separator))
                ...
                for Eq in ps.equipment_list:
                    ...
                    if Eq[1] == 1 or Eq[1] == 42:
                        (
                            ...
                            KVATot,
                            Conn,
                        ) = ps.query_device(ps.xfmr_equipment, Eq[2], Eq[1])
                        textfile.write("\n{}".format(XfmrType))
                    ...
                    # By-phase Transformer
                        ...
                        (
                            ...
                            Conn,
                            KVANom,
                        ) = ps.query_device(ps.xfmr_equipment_by_phase, Eq[2], Eq[1])
                    ...
                    # Series Reactor
                        ...
                        textfile.write("\nSeries Reactor")
                        RatedCurrent, ReactanceOhms = ps.get_value_eq(
                    ...
                    # UG Cable
                        ...
                        textfile.write("\nUnderground Cable")
                        R1, X1, R0, X0, ImpedancesNote, Comments = ps.get_value_eq(
                    ...
                    # Source Equivalent
                        ...
                        R1max, X1max, R0max, X0max, R1min, X1min, R0min, X0min = (
                            ps.query_node(ps.source_info, ps.source_name)
                        )
                        textfile.write(
    ...
    # The class initializes with all attributes set to default values
        ActivateModifications(True)

    @staticmethod
    def query_node(keyword_list, node_id_list):
        #        """
        #        Queries information for a list of nodes based on given identifiers and node string.
        #
        #        Args:
        #        list_id (list): A list of node identifiers.
        #        node_str (str): A string representing the node attribute to query.
        #
        #        Returns:
        #        list: A list of queried information for each node.
        #        """
        try:
            return [locale.atof(QueryInfoNode(id, node_id_list)) for id in keyword_list]
        except:
            output_list = []
            for id in keyword_list:
                try:
                    output_list.append(locale.atof(
                        QueryInfoNode(id, node_id_list, 5)))
                except:
                    output_list.append(QueryInfoNode(id, node_id_list, 5))
            return output_list

    @staticmethod
    def query_device(keyword_list, dev_number_list, dev_type_list):
        #        """
        #        Queries information for a list of devices based on identifiers, device number, and type.
        #
        #        Args:
        #        list_id (list): A list of device identifiers.
        #        dev_number (int): The number associated with the device.
        #        dev_type (int): The type of the device.
        #
        #        Returns:
        #        list: A list of queried information for each device.
        #        """
        try:
            return [
                locale.atof(QueryInfoDevice(id, dev_number_list, dev_type_list)) for id in keyword_list
            ]
        except:
            output_list = []
            for id in keyword_list:
                try:
                    output_list.append(
                        locale.atof(QueryInfoDevice(
                            id, dev_number_list, dev_type_list, 5))
                    )
                except:
                    output_list.append(QueryInfoDevice(
                        id, dev_number_list, dev_type_list, 5))
            return output_list

    @staticmethod
    def get_value_eq(keyword_list, eq_name_list, eq_type_list):
        #        """
        #        Retrieves values for a list of equipment based on identifiers, equipment name, and type.
        #
        #        Args:
        #        list_id (list): A list of equipment identifiers.
        #        eq_name (str): The name of the equipment attribute to query.
        #        eq_type (int): The type of the equipment.
        #
        #        Returns:
        #        list: A list of values for each piece of equipment.
        #        """
        try:
            return [locale.atof(Eqt.GetValue(id, eq_name_list, eq_type_list))for id in keyword_list]
        except:
            output_list = []
            for id in keyword_list:
                try:
                    output_list.append(locale.atof(
                        Eqt.GetValue(id, eq_name_list, eq_type_list)))
                except:
                    output_list.append(Eqt.GetValue(
                        id, eq_name_list, eq_type_list))
            return output_list

    @staticmethod
    def set_value_source(value_property_setlist, source_id):
        for set in value_property_setlist:
            for value, property in set:
