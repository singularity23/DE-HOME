window.onload = () => {
  ...
  var htmlCode = '';
  const htmlRender = document.querySelector('#WebPartWPQ6 .ms-rtestate-field');
  ...
  const renderHTML = (name, dataInfo, idx) => {
    ...
    htmlCode = `<section>` + state.boxList.map(box => box.innerHTML).join('') + `</section>`;
  ...
  const transformRecord = line => {
    ...
    let md = link
      ...
      : `${indent} ${pretext}${desc ? `<abbr title="${desc}">${code}</abbr>` : code}${trailing}${openTag}${closeTag}`;
