document.getElementById('searchButton').addEventListener('click', () => {
  ...
  const S = (E) => {
    ...
    if (
      ((T +=
        { G: 'GND ', P: 'PHS ', Q: 'NEG ', N: 'GND ' }[E.charAt(2)] || 'PHS '),
      /^50P[234]/.test(E))
    )
  ...
  N = () => {
    const T = Array.from(_C1MVCCtrl5._ncc),
      S = T.length
    if (S > 10) for (let N = 0; N < S; N++) T[N][4] = E(T[N][2])
