...
SELECT
  ...
  CASE
    ...
    AND T.SETTINGNAME NOT LIKE '%79OI%'
    AND S.GROUPNAME = '1'
...
WHERE ${baseCondition}
  ...
  const SearchComponent = {
    ...
    async _handleSearch (inputEl) {
      ...
      if (!regexValidator(CONFIG.patterns.searchInput, searchValue)) {
  ...
  const DataProcessor = {
    ...
    async getQueryResults (fingerPrint, startTime, status) {
      return new Promise((resolve, reject) => {
        debugLog('Creating new finger print');
