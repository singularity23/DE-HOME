javascript: (function () {
  ...
  const MyConsole = {
    // Single timestamped emitter so log/debug can't drift apart.
    _emit (...args) {
    ...
    debug (...args) {
      if (CONFIG.debug) this._emit(...args);
    },
          ...
          SELECT R.S01 AS DEVICE,
            ...
            TO_NUMBER(
              COALESCE(
                ...
                REGEXP_SUBSTR(TE.GROUPNAME, 'PARAMETER SUBSET (\d+)', 1, 1, NULL, 1)
              )            
            ) AS SUBSET_NUM
          FROM TRELAY R
        ...
        const enableSettings = `
          enable_settings AS (
            ...
            WHERE SE.GROUPNAME LIKE 'PARAMETER%'
              AND (TE.SETTINGNAME LIKE '%SUBSET _/F<>/ENABLE%' OR
              TE.SETTINGNAME LIKE '%-ENABLE%')
  ...
  const Utils = {
    ...
    validatePattern (pattern, value) {
      ...
      try {
        // Coerce first, then trim — never throws when value is null/undefined/number.
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
          try {
            ...
            if (attempt < maxRetries) {
              ...
              await new Promise((resolve) => setTimeout(resolve, delay * (attempt + 1))); // Linear backoff (delay grows by `delay` each attempt)
            }
          }
        }
        throw lastError;
  ...
  const SearchComponent = {
    ...
    async init () {
      ...
      try {
        // Re-running the bookmarklet refreshes the UI: if a bar already exists,
        // tear it down (handlers + DOM) and rebuild from scratch, so every run
        // lands on a working search bar rather than a dead one.
        if (this._initialized) {
          MyConsole.debug('Search component already present — rebuilding');
          this._teardownUI();
        }
    ...
    /**
     * Tears down the existing search bar (event handlers + DOM node) so init()
     * can rebuild it cleanly. _createSearchUI early-returns when the container
     * still exists, so the node must be removed here for a refresh to take.
     * @private
     */
    _teardownUI () {
      this._cleanupEventHandlers();
      const container = DOM.getElement(CONFIG.selectors.searchContainer, true);
      container?.remove();
    ...
    async _handleSearch (inputEl, status = null) {
      ...
      try {
        ...
        DOM.clearAll();
        // Reset the adaptive query-delay budget. It grows by 2s on each timeout
        // (see DataProcessor.getQueryResults); without this reset a slow earlier
        // search would permanently inflate the threshold for every later one.
        DataProcessor._queryDelay = 0;
    ...
    async _executeStatusSearch (searchValue, status) {
      ...
      try {
        ...
        const getQueryResultsWithRetry = Utils.withRetry(
          () => {
            // Arrow functions have no own `arguments`; the previous spread logged
            // the enclosing method's args by accident. Log the captured values.
            MyConsole.debug('getQueryResultsWithRetry:', { fingerPrint, status });
  ...
  const DataProcessor = {
    ...
    _handleProcessingFailure (originalError) {
      ...
      const headRow = StateManager.get('headerRow');
      const tableRows = StateManager.get('tableRows');
     ...
     */
    _rearrangeTableData () {
      const headerRow = StateManager.get('headerRow');
      let tableRows = StateManager.get('tableRows');

      const relayType = StateManager.get('relayType');
    ...
    _validateData () {
      ...
      // Arrays are always truthy, so the old `if (headerRow && tableRows)` never
      // actually gated anything — use a real emptiness check and the local copies.
      if (Utils.hasElements(headerRow) && Utils.hasElements(tableRows)) {
        const expected = headerRow.length;
        const inconsistent = tableRows.filter((r) => Utils.toArray(r).length !== expected).length;
  ...
  const SELDecoder = {
    ...
    decode (settingName, settingValue) {
      ...
      try {
        ...
        if (this.tripEquation.pattern.test(settingName) || String(settingValue).split('+').length > 1) return [this.tripEquation.desc, settingValue];
    ...
    _createBody (rows) {
      ...
      const trElements = rows.map((row) => {
        ...
        const tdElements = rowData.map((cell) => {
          // `typeof null === 'object'`, so guard explicitly: raw (unmerged) rows
          // can hold null/undefined cells (e.g. an empty MEMO column), which would
          // otherwise throw on `cell.value` / `cell.merged` and abort rendering.
          const isCellObject = cell !== null && typeof cell === 'object';
          const content = isCellObject ? cell.value : cell;
          const td = DOM.createElement('td', {
            textContent: content,
            classList: isCellObject && cell.merged ? 'aspen-cell merged' : 'aspen-cell',
          });

          if (isCellObject && cell.rowspan > 1) {
