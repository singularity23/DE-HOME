javascript: (function () {
	...
	const removeRows = (table, tripElement) => {
		Array.from(table.rows).forEach((row) => {
			...
			if (row.innerText.trim() === "") {
				row.remove();
			} else if (cells[2].innerText.trim() === "TR" && tripElement !== cells[3].innerText.trim()) {
	...
	const handleMech = (Table, Rows) => {
		...
		Array.from(Rows).forEach((row) => {
			Array.from(row.cells).forEach((cell, index) => {
				...
				textInfo += cell.textContent + (index % 5 === 4 ? createSeparator() + "\n" : "\n");
			});
			row.remove();
		});
		...
		Array.from(table.rows).forEach((row) => {
			if (row.id != "highlight") {
				...
				if (cells.length >= 2) {
					...
					if (equations.indexOf(elementCell) === -1 && !tcEle.includes(elementCell)) {
						if (defEle[elementCell] && equations.includes(defEle[elementCell])) {
	...
	const getDeviceNumber = (elementText) => {
		...
		if (/^50P[234]/.test(elementText)) {
			...
			const suffixMap = { P: "Pick Up(A)", C: "Curve", TD: "Time Dial(s)", TC: "Torque Control" };
